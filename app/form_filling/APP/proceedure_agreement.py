...
class ProcedureAgreementForm:

    def __init__(self):
        self.input_file_path = os.path.join('static/PDFs', 'residential_sale.pdf')
        self.output_file_path = os.path.join('media', 'procedure_agreement.pdf')
    ...
    def write_to_file(self, data):
        # fields = fillpdf.fillpdfs.get_form_fields(self.input_file_path, page_number=16)
        # print(fields)
        content = {
            "Property Jdr5": data.get('property_address'),
            "Client Nuncs": data.get('client_name'),
            "Seller[0]": data.get('seller_1_name'),
            "Dou": data.get('seller_1_date'),
            "Seller_2": data.get('seller_2_name'),
            "undefined": data.get('seller_2_date', '20'),
            "Sales RcprscnrativcBrokcr Aurhorkfd to 1i11d tht Broktmgc": data.get('broker_name'),
            "luc": data.get('broker_date')
        }

        self.writer.updatePageFormFieldValues(self.writer.getPage(15), content)
        ...
        get_write_page(self.output_file_path, 15)
